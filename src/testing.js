...
import { testFailStyle, testPassStyle, testTitleStyle } from "./output/consoleThemes.js";

function buildTestingContext(mavka) {
  const context = new mavka.Context(mavka, mavka.context);

  context.set("тестовий_випадок", mavka.makeProxyFunction((args, context) => {
    if (!mavka.isText(args[0])) {
      mavka.fall(context, mavka.makeText("Очікується, що перший параметр є текст."));
    }

    if (!mavka.isDiia(args[1])) {
      mavka.fall(context, mavka.makeText("Очікується, що другий параметр є Дія."));
    }
    ...
    try {
      ...
      console.log(testPassStyle("✔ успішно"), testTitleStyle(args[0].asJsValue()) + "\n")
    } catch (e) {
      if (e instanceof mavka.ThrowValue) {
        console.error(testFailStyle("✖ провалено"), testTitleStyle(args[0].asJsValue(context)));
